...
SELECT 
    page_views,
    add_to_carts,
    ROUND(100.0 * add_to_carts / page_views, 2) as add_to_cart_rate,
    checkouts,
    ROUND(100.0 * checkouts / add_to_carts, 2) as checkout_rate,
    payment_info,
    ROUND(100.0 * payment_info / checkouts, 2) as payment_info_rate,
    purchases,
    ROUND(100.0 * purchases / payment_info, 2) as purchase_rate,
    ROUND(100.0 * purchases / page_views, 2) as overall_conversion_rate
FROM funnel_counts;
...
SELECT 
    ...
    ROUND(100.0 * (step1 - step2) / step1, 2) as drop_off_rate
FROM funnel_steps
UNION ALL
SELECT 
    'Add to Cart → Checkout',
    step2 - step3,
    ROUND(100.0 * (step2 - step3) / step2, 2)
FROM funnel_steps
UNION ALL
SELECT 
    'Checkout → Payment Info',
    step3 - step4,
    ROUND(100.0 * (step3 - step4) / step3, 2)
FROM funnel_steps
UNION ALL
SELECT 
    'Payment Info → Purchase',
    step4 - step5,
    ROUND(100.0 * (step4 - step5) / step4, 2)
FROM funnel_steps
...
SELECT 
    ...
    ROUND(100.0 * COUNT(*) / SUM(COUNT(*)) OVER(), 2) as pct_of_purchases
FROM user_events
...
SELECT 
    DATE(event_date) as purchase_date,
...
SELECT 
    ...
    ROUND(100.0 * (carts - purchases) / carts, 2) as abandonment_rate
FROM cart_stats
WHERE carts > 0
...
SELECT 
    ...
    ROUND(100.0 * purchasing_users / total_users, 2) as purchase_rate
FROM user_activity;
...
SELECT 
    ...
    total_users,
    purchasers,
    ROUND(100.0 * purchasers / total_users, 2) as conversion_rate,
    ROUND(revenue, 2) as total_revenue,
    ROUND(revenue / total_users, 2) as revenue_per_user
FROM cohort_metrics
...
SELECT 
    ...
    ROUND(100.0 * COUNT(DISTINCT CASE WHEN event_type = 'purchase' THEN user_id END) / COUNT(DISTINCT user_id), 2) as daily_conversion_rate
FROM user_events
GROUP BY DATE(event_date)
ORDER BY date;
...
SELECT 
    ...
    COUNT(CASE WHEN event_type = 'purchase' THEN 1 END) as purchases,
    ROUND(SUM(CASE WHEN event_type = 'purchase' THEN amount ELSE 0 END), 2) as revenue
FROM user_events
...
SELECT 
    ...
    users,
    buyers,
    ROUND(100.0 * buyers / users, 2) as conversion_rate,
    ROUND(revenue, 2) as revenue,
    ROUND(revenue / users, 2) as revenue_per_user,
    CASE 
        WHEN revenue / users > 20 THEN 'High Priority - Increase Budget'
        WHEN revenue / users > 15 THEN 'Medium Priority - Maintain'
...
SELECT 
    ...
    carts,
    purchases,
    ROUND(100.0 * purchases / NULLIF(views, 0), 2) as conversion_rate,
    ROUND(100.0 * (carts - purchases) / NULLIF(carts, 0), 2) as cart_abandonment_rate,
    ROUND(revenue, 2) as revenue,
    CASE 
        WHEN purchases / NULLIF(views, 0) < 0.10 THEN 'Review product page & pricing'
        WHEN (carts - purchases) / NULLIF(carts, 0) > 0.50 THEN 'High cart abandonment - check checkout flow'
